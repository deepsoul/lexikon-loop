...
const multiplayer = {
  ...
  // UI State
  showJoinModal: ref(false),
  joinPlayerName: ref(''),
  multiplayerStatusText: ref(''),
  multiplayerStatusClass: ref(''),
  scannerActive: ref(false),
  scannerStatus: ref(''),
  ...
  async startHost() {
    try {
      ...
      this.isHost.value = true;
      this.isClient.value = false;
      this.roomId.value = this.generateRoomId();
      this.hostId.value = this.roomId.value;

      console.log('🏠 Room ID:', this.roomId.value);
      ...
      // Generate QR code
      const connectionUrl = `${window.location.origin}/join?hostId=${this.roomId.value}`;
      await this.generateQRCode(connectionUrl);

      this.multiplayerStatusText.value =
        'Host gestartet - Warte auf Spieler...';
      this.multiplayerStatusClass.value = 'status-success';
      playSound('success');
    } catch (error) {
      console.error('❌ Host start error:', error);
      this.multiplayerStatusText.value = 'Host Fehler';
      this.multiplayerStatusClass.value = 'status-error';
      playSound('timer');
    }
  },

  // Initialize multiplayer as client
  async startClient(hostId: string, playerName: string) {
    try {
      console.log('👤 Starting multiplayer client...');

      this.isHost.value = false;
      this.isClient.value = true;
      this.hostId.value = hostId;
      this.roomId.value = hostId;

      console.log('🏠 Joining room:', this.roomId.value);
      console.log('👤 Player name:', playerName);
      ...
      this.multiplayerStatusText.value = 'Verbinde mit Host...';
      this.multiplayerStatusClass.value = 'status-info';
    } catch (error) {
      console.error('❌ Client start error:', error);
      this.multiplayerStatusText.value = 'Verbindungsfehler';
      this.multiplayerStatusClass.value = 'status-error';
      playSound('timer');
    }
  },
  ...
  setupSocketHandlers() {
    ...
    this.socket.on('disconnect', () => {
      ...
      this.multiplayerStatusText.value = 'Verbindung verloren';
      this.multiplayerStatusClass.value = 'status-error';
    });
    ...
    this.socket.on('playerJoined', (data) => {
      console.log('👥 Player joined:', data);
      this.players.value = data.allPlayers;
      this.gameState.value = data.gameState;

      if (this.isHost.value) {
        this.multiplayerStatusText.value = `${data.player.name} ist beigetreten!`;
        this.multiplayerStatusClass.value = 'status-success';
      }
    });

    this.socket.on('diceRolled', (gameState) => {
      console.log('🎲 === DICE ROLLED EVENT ===');
      console.log('📊 Game state:', gameState);
      console.log('🏠 Is host:', this.isHost.value);
      console.log('👤 Is client:', this.isClient.value);

      this.handleDiceRolled(gameState);
    });

    this.socket.on('diceStopped', (gameState) => {
      console.log('🛑 === DICE STOPPED EVENT ===');
      console.log('📊 Final game state:', gameState);

      this.handleDiceStopped(gameState);
    });
  ...
  handleDiceRolled(gameState: any) {
    try {
      nextTick(() => {
        // Clear previous results
        resultText.value = '';
      ...
      console.error('❌ Error in diceRolled handler:', error);
    }
  },

  // Handle dice stop event
  handleDiceStopped(gameState: any) {
    try {
      nextTick(() => {
        rolling.value = false;
        ...
        if (gameState.category) {
          this.setFinalDicePosition(gameState.category);
        }
      });
    } catch (error) {
      console.error('❌ Error in diceStopped handler:', error);
    }
  },
  ...
  // Set final dice position
  setFinalDicePosition(categoryName: string) {
    try {
      const categoryIndex = categories.findIndex(
      ...
      if (categoryIndex !== -1) {
        ...
        nextTick(() => {
          ...
          console.log('🎯 Set dice to final position for:', categoryName);
        });
      }
    } catch (error) {
      console.error('❌ Error setting dice position:', error);
  ...
  rollDice() {
    ...
    console.log('🏠 Room ID:', this.roomId.value);
    console.log('👥 Players:', this.players.value?.length || 0);

    if (this.socket && this.roomId.value && this.socket.connected) {
      rolling.value = true;
      playSound('roll');

      this.socket.emit('rollDice', {roomId: this.roomId.value});
      console.log('📡 Dice roll sent to server');
    } else {
      console.log('❌ Cannot roll dice - socket not ready');
      // Fallback to single player
      this.rollDiceSinglePlayer();
    }
  },

  // Single player dice roll (fallback)
  rollDiceSinglePlayer() {
    console.log('🎮 Single player dice roll');
    ...
    // Clear results during animation
    resultText.value = '';
    subResult.value = '';
    currentLetter.value = '-';
  ...
  async generateQRCode(data: string) {
    try {
      ...
      this.showTextFallback(data);
    }
  },

  // Show text fallback
  showTextFallback(data: string) {
    if (qrCodeRef.value) {
      qrCodeRef.value.innerHTML = `
        <div style="padding: 20px; text-align: center; color: #64748b; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
          <div style="margin-bottom: 10px;">
  ...
  disconnect() {
    ...
    this.multiplayerStatusText.value = '';
    this.multiplayerStatusClass.value = '';
...
function joinMultiplayerGame() {
  const playerName =
    multiplayer.joinPlayerName.value.trim() ||
    `Spieler ${Math.floor(Math.random() * 1000)}`;
  multiplayer.startClient(multiplayer.hostId.value, playerName);
}
...
// Override rollDice function to use multiplayer
function rollDice() {
  if (multiplayer.isHost.value || multiplayer.isClient.value) {
    multiplayer.rollDice();
  } else {
